          ...
          <div className="p-5">
            ...
            <div className="flex items-center gap-3 mt-4 flex-wrap">
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                参加 {attendCount}人
                {nextSchedule.capacity !== null && <span className="text-gray-400 font-normal ml-0.5">/{nextSchedule.capacity}</span>}
              </Badge>
              <span className="text-xs text-gray-400">/ {memberCount}人中</span>
              {nextSchedule.capacity !== null && attendCount >= nextSchedule.capacity && (
                <Badge className="bg-red-50 text-red-700 border-red-200 text-[10px]">満員</Badge>
              )}
            </div>
          ...
          {isHost && (
            <>
              ...
              <QuickAction
                ...
                label="出席率"
                color="bg-rose-50"
                icon={<svg className="w-5 h-5 text-rose-600" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>}
              />
            </>
          )}
        </div>
